	...
	eventsSubscriber := func(event datatransfer.Event, channelState datatransfer.ChannelState) {
		...
		case datatransfer.DataReceivedProgress:
			...
			if totalReceived > lastReceived {
				shared.sendEvent(events.DataReceived(retrieval.Clock.Now(), retrieval.request.RetrievalID, candidate, multicodec.TransportGraphsyncFilecoinv1, totalReceived-lastReceived))
			}
