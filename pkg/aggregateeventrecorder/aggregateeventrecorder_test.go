	...
	tests := []struct {
	...
	}{
		{
			...
			exec: func(t *testing.T, ctx context.Context, subscriber types.RetrievalEventSubscriber, id types.RetrievalID) {
				...
				subscriber(events.DataReceived(clock.Now(), id, bitswapCandidates[0], multicodec.TransportBitswap, 3000))
				subscriber(events.DataReceived(clock.Now(), id, bitswapCandidates[0], multicodec.TransportBitswap, 2000))
				...
				subscriber(events.DataReceived(clock.Now(), id, graphsyncCandidates[1], multicodec.TransportGraphsyncFilecoinv1, 3000))
				subscriber(events.DataReceived(clock.Now(), id, graphsyncCandidates[1], multicodec.TransportGraphsyncFilecoinv1, 2000))

				subscriber(events.DataReceived(clock.Now(), id, bitswapCandidates[1], multicodec.TransportBitswap, 5000))
